	...
	class YITH_WCAS_Gb_Empty_Block extends Abstract_YITH_WCAS_Gb_InnerBlock {
		...
		public function add_attributes( $attributes, $block ) {
			if ( $this->get_block_type() === $block['blockName'] ) {
				...
				$attributes['totalInnerBlocks'] = $inner_blocks;
				$attributes['hasHistoryBlock']  = $inner_blocks > 0 && $this->has_inner_block( 'yith/history-block', $block['innerBlocks'] ) ? 'yes' : 'no';
				$attributes['hasPopularBlock']  = $inner_blocks > 0 && $this->has_inner_block( 'yith/popular-block', $block['innerBlocks'] ) ? 'yes' : 'no';
			}
