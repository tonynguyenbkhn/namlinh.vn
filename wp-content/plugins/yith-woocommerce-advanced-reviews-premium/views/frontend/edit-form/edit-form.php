	...
	<div class="form-content">
		<?php if ( 'review' === $type ) : ?>
			<div class="form-element form-review-rating">
				...
				if ( $multi_criteria_enabled ) {
					...
					$args = array(
						'label'      => esc_html_x( 'Rating', '[Global] Generic text', 'yith-woocommerce-advanced-reviews' ),
		...
		<?php if ( ! $logged_user && 'create' === $action ) : ?>
			<div class="form-element form-review-user-data">
				<label>
					<?php echo esc_html_x( 'Your Name', '[Frontend] User name form field', 'yith-woocommerce-advanced-reviews' ); ?>:
					<input type="text" name="yith-ywar-user-name" value="<?php echo esc_html( $review ? $review->get_review_author() : '' ); ?>"/>
				</label>
				<label>
					<?php echo esc_html_x( 'Your Email', '[Frontend] User email form field', 'yith-woocommerce-advanced-reviews' ); ?>:
		...
		<?php if ( yith_ywar_check_user_permissions( 'title-reviews' ) ) : ?>
			<div class="form-element form-review-title">
				<label>
					<?php echo 'review' === $type ? esc_html_x( 'Review title', '[Frontend] Review title form field', 'yith-woocommerce-advanced-reviews' ) : esc_html_x( 'Reply title', '[Frontend] Reply title form field', 'yith-woocommerce-advanced-reviews' ); ?>:
		...
		<div class="form-element form-review-content">
			<label>
				<?php echo 'review' === $type ? esc_html_x( 'Your review', '[Frontend] Review content form field', 'yith-woocommerce-advanced-reviews' ) : esc_html_x( 'Your reply', '[Frontend] Reply content form field', 'yith-woocommerce-advanced-reviews' ); ?>:
		...
		<?php if ( $upload_images || $upload_video ) : ?>
			<div class="form-element form-review-attachment">
				<label>
					<?php echo esc_html_x( 'Upload files', '[Frontend] Review upload field label', 'yith-woocommerce-advanced-reviews' ); ?>:
					<span>
						<?php
						switch ( true ) {
							case $upload_images && ! $upload_video:
								/* translators: %s number of items */
								$file_amount = sprintf( esc_html_x( 'You can upload a maximum of %s images.', '[Frontend] Review upload field file label', 'yith-woocommerce-advanced-reviews' ), yith_ywar_get_option( 'ywar_max_attachments' ) );
								break;
							case ! $upload_images && $upload_video:
								/* translators: %s number of items */
								$file_amount = sprintf( esc_html_x( 'You can upload a maximum of %s videos.', '[Frontend] Review upload field file label', 'yith-woocommerce-advanced-reviews' ), yith_ywar_get_option( 'ywar_max_attachments_video' ) );
								break;
							default:
								/* translators: %1$s number of images, %2$s number of videos */
								$file_amount = sprintf( esc_html_x( 'You can upload a maximum of %1$s images and %2$s videos.', '[Frontend] Review upload field file label', 'yith-woocommerce-advanced-reviews' ), yith_ywar_get_option( 'ywar_max_attachments' ), yith_ywar_get_option( 'ywar_max_attachments_video' ) );
						}

						/* translators: %s list of allowed file formats */
						$allowed_types = sprintf( esc_html_x( 'Allowed file types: %s.', '[Frontend] Review upload field file types label', 'yith-woocommerce-advanced-reviews' ), esc_html( yith_ywar_get_allowed_filetypes() ) );

						switch ( true ) {
							case $upload_images && ! $upload_video:
								/* translators: %s file size */
								$file_size = sprintf( esc_html_x( 'Maximum file size: %sMB.', '[Frontend] Review upload field file size label', 'yith-woocommerce-advanced-reviews' ), yith_ywar_get_option( 'ywar_attachment_max_size' ) );
								break;
							case ! $upload_images && $upload_video:
								/* translators: %s file size */
								$file_size = sprintf( esc_html_x( 'Maximum file size: %sMB.', '[Frontend] Review upload field file size label', 'yith-woocommerce-advanced-reviews' ), yith_ywar_get_option( 'ywar_attachment_max_size_video' ) );
								break;
							default:
								/* translators: %1$s image size, %2$s video size */
								$file_size = sprintf( esc_html_x( 'Maximum file size: %1$sMB for images and %2$sMB for videos.', '[Frontend] Review upload field file size label', 'yith-woocommerce-advanced-reviews' ), yith_ywar_get_option( 'ywar_max_attachments' ), yith_ywar_get_option( 'ywar_max_attachments_video' ) );
						}
	...
	<div class="form-footer">
		<?php if ( $logged_user ) : ?>
			<span class="login-info">
				...
				printf( esc_html_x( 'Logged in as %s.', '[Frontend] Logged user info', 'yith-woocommerce-advanced-reviews' ), esc_html( wp_get_current_user()->display_name ) );
				?>
				<a href="<?php echo esc_url( wc_logout_url() ); ?>"><?php echo esc_html_x( 'Logout', '[Frontend] Logout form link', 'yith-woocommerce-advanced-reviews' ); ?> &gt;</a>
			</span>
		<?php else : ?>
