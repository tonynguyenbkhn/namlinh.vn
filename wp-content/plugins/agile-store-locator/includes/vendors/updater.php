...
/**
 * Plugin Updater Class for Agile Store Locator.
 * Handles checking for updates and updating the plugin.
 */
class Updater {
  ...
  public function check_update( $_transient_data ) {
    ...
    // Check if last API request was within the past hour
    $last_api_request_time = get_option('asl_last_api_request_time');

    if ($last_api_request_time && (time() - $last_api_request_time) < HOUR_IN_SECONDS) {
      return $_transient_data; // Skip API call if it's too soon
    }
    ...
    // Record the time of the API request
    update_option('asl_last_api_request_time', time());

    $this->api_data['license'] = $this->getDetails();

    //  Check if license is valid
    $license_check = $this->check_site_license($this->api_data['license']);

    
    if ($license_check->license != 'error') {

      // If the license is valid or the site is inactive, set the status to valid
      if ($license_check->license == 'valid' || $license_check->license == 'site_inactive') {
          set_transient('asl_license_status', 'valid', WEEK_IN_SECONDS); // Cache for 1 week
      }
      else {
        
        // If the license is anything other than valid or site inactive, delete the asl-compatible option
        delete_option('asl-compatible'); // Remove option if the license is invalid
        set_transient('asl_license_status', 'invalid', WEEK_IN_SECONDS); // Cache for 1 week
      }
    }
    else {
      set_transient('asl_license_status', 'error', WEEK_IN_SECONDS); // Cache for 1 day
    }
  ...
  /**
   * [getDetails Return the details about the license]
   * @return [type] [description]
   */
  public function getDetails() {

    // Retrieve the saved license details from the WordPress options
    $license_details = get_option('asl-compatible');

    // Check if the license details and 'purchase-code' exist, then return it
    if (isset($license_details['purchase-code']) && !empty($license_details['purchase-code'])) {

        // Validate the format of the purchase code (UUID format)
        $purchase_code = $license_details['purchase-code'];
        $key_format_pattern = '/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/';

        if (preg_match($key_format_pattern, $purchase_code)) {
            return $purchase_code;  // Return the valid license key
        }
    }

    // Return an empty string if no valid license key is found
    return '';
  }
  ...
  public function show_update_notification( $file, $plugin ) {
    ...
    printf(
      ...
      esc_html__( 'There is a new version of %1$s available.', 'asl_locator' ),
      esc_html( $plugin['Name'] )
    );

    if ( ! current_user_can( 'update_plugins' ) ) {
      echo ' ';
      esc_html_e( 'Contact your network administrator to install the update.', 'asl_locator' );
    } elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
      echo ' ';
      printf(
        /* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
        __( '%1$sView version %2$s details%3$s.', 'asl_locator' ),
      ...
      echo ' ';
      printf(
        __( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'asl_locator' ),
        '<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
        esc_html( $update_cache->response[ $this->name ]->new_version ),
        '</a>',
      ...
      printf(
        ' %1$s%2$s%3$s',
        '<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
        esc_html__( 'Update now.', 'asl_locator' ),
        '</a>'
      );
    }
  ...
  public function show_changelog() {
    ...
    if ( ! current_user_can( 'update_plugins' ) ) {
      wp_die( esc_html__( 'You do not have permission to install plugin updates', 'asl_locator' ), esc_html__( 'Error', 'asl_locator' ), array( 'response' => 403 ) );
    }
  ...
  public function set_version_info_cache( $value = '', $cache_key = '' ) {
    ...
    $data = array(
      'timeout' => strtotime( '+36 hours', time() ), // Increased cache duration to 12 hours
  ...
  /**
   * Check Site license from server
   *
   * @since 1.9.0
   * @return string
   */
  private function check_site_license($license_key) {
    
    $url = ASL_UPDATE_URL.'/?edd_action=check_license&item_id='.ASL_PLUGIN_ITEM_ID.'&license='.$license_key.'&url='.preg_replace('#^https?://#', '', site_url());
