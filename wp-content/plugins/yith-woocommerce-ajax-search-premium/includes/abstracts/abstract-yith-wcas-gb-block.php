...
abstract class Abstract_YITH_WCAS_Gb_Block {
	...
	protected function register_chunk_translations( $chunks ) {
		foreach ( $chunks as $chunk ) {
			...
			$this->register_script( $handle, $this->get_block_asset_build_path( $chunk ), array(), true );
	...
	protected function get_block_type_editor_script( $key = null ) {
		$script = array(
			...
			'dependencies' => array( 'ywcas-blocks', 'accounting' ),
		);
	...
	protected function get_block_type_style() {
		...
		$this->register_style( 'ywcas-blocks-style-frontend', $this->get_block_asset_build_path( 'frontend', 'css' ), array(), 'all' );
