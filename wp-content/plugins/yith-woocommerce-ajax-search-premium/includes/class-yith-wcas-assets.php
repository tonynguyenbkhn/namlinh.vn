	...
	class YITH_WCAS_Assets {
		...
		public static function load_admin_scripts( $hook ) {
			...
			if ( 'yith-plugins_page_yith_wcas_panel' === $hook ) {
				...
				if ( isset( $_GET['tab'] ) ) { //phpcs:ignore
					...
					if ( isset( $_GET['sub_tab'] ) ) {
						$sub_tab = sanitize_text_field( wp_unslash( $_GET['sub_tab'] ) ); //phpcs:ignore
						if ( 'boost' === $tab && 'boost-boost-product' === $sub_tab ) {
							self::enqueue_script( 'ywcas-boost-product' );
						}
					}
				} else {
					self::enqueue_script( 'ywcas-statistic' );
			...
			if ( isset( $_GET['post_type'] ) ) {
				...
				if ( 'ywcas_boost' === $post_type ) {
					self::enqueue_style( 'ywcas-admin' );
					self::enqueue_script( 'ywcas-boost-rule' );
				}
			}
		...
		public static function register_backend_script() {
			$admin_scripts = array(
				...
				'ywcas-boost-product' => array(
					'src'     => yit_load_js_file( YITH_WCAS_ASSETS_URL . '/js/admin/build/boost-product.js' ),
					'deps'    => array( 'jquery', 'jquery-blockui' ),
					'version' => YITH_WCAS_VERSION,
				),
				'ywcas-boost-rule'    => array(
					'src'     => yit_load_js_file( YITH_WCAS_ASSETS_URL . '/js/admin/build/boost-rule.js' ),
					'deps'    => array( 'jquery', 'jquery-blockui', 'wp-util', 'wc-enhanced-select' ),
					'version' => YITH_WCAS_VERSION,
				),
			);
		...
		private static function get_script_data( $handle ) {
			switch ( $handle ) {
				case 'ywcas-admin':
					...
					break;
				case 'ywcas-boost-product':
					$params = array(
						'ajaxurl'            => admin_url( 'admin-ajax.php' ),
						'searchProductNonce' => wp_create_nonce( 'ywcas-search-product' ),
						'modalTitle'         => _x( 'Choose products to boost', 'Modal title', 'yith-woocommerce-ajax-search' ),
						'confirm_message'    => array(
							'title'         => __( 'Are you sure?', 'yith-woocommerce-ajax-search' ),
							'desc'          => __( 'Do you want to remove this boost?', 'yith-woocommerce-ajax-search' ),
							'confirmButton' => __( 'Yes, proceed', 'yith-woocommerce-ajax-search' ),
						),
					);
					break;
				case 'ywcas-boost-rule':
					$params = array(
						'ajaxurl'                => admin_url( 'admin-ajax.php' ),
						'loadBoostTemplateNonce' => wp_create_nonce( 'ywcas-load-boost-rule-template' ),
						'saveRuleNonce'          => wp_create_nonce( 'ywcas-save-boost-rule' ),
						'editRuleNonce'          => wp_create_nonce( 'ywcas-edit-boost-rule' ),
						'editInLineRuleNonce'    => wp_create_nonce( 'ywcas-edit-in-line-boost-rule' ),
						'requiredError'          => __( 'This field is required', 'yith-woocommerce-ajax-search' ),
						'newRuleTitle'           => __( 'New boost rule', 'yith-woocommerce-ajax-search' ),
						'editRuleTitle'          => __( 'Edit boost rule', 'yith-woocommerce-ajax-search' ),
					);
					break;
				default:
					$params = false;
