	...
	class CSF_Field_image_sizes extends CSF_Fields {
		...
		public function render() {
			...
			$sizes = array_merge(
				$sizes,
				array(
					'full'             => __( 'Original uploaded image', 'wpgs' ),
					'pgs_custom_image' => __( 'Set custom size', 'wpgs' ),
				)
			);
			...
			if ( ! empty( $sizes ) ) {
				...
				if ( ! empty( $sizes ) ) {
					...
					echo ! empty( $this->field['empty_message'] ) ? esc_html( $this->field['empty_message'] ) : esc_html__( 'No image sizes found.', 'wpgs' );
